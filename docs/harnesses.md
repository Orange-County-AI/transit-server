...
The daemon falls back to Herdr `agent.prompt`, which needs Herdr 0.8.2 or newer with `herdr.service` running. Herdr injection drives a PTY: it scrapes the pane to guess whether a composer is mid-draft, targets a terminal container rather than a session, and cannot distinguish a resumed session from a new one. Native adapters avoid those failure modes because their harnesses can state which session owns the delivery.
